...
public class TLogin extends JFrame implements ActionListener{
    ...
    public void actionPerformed(ActionEvent ae){
        ...
        try{
            conn sc1 = new conn();
            String su = st1.getText();
            String sv = st2.getText();
            
            String sq = "select * from login where username='"+su+"' and password='"+sv+"'";
            
            ResultSet rs = sc1.s.executeQuery(sq); 
            if(rs.next()){
                new TProject().setVisible(true);
                setVisible(false);
            }else{
